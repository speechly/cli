...
func main() {
	...
	var s string
	for _, f := range files {
		if strings.Contains(f.Name(), "README.md") || strings.Contains(f.Name(), "speechly.md") || strings.Contains(f.Name(), "speechly_projects.md") {
			continue
		}
	...
	if err := os.WriteFile(reference, []byte(s), 0644); err != nil {
		log.Fatal(err)
	}
}
